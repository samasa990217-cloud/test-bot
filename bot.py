...
def run_flask():
    app.run(host="0.0.0.0", port=10000)  # Render Web Service 會自動 ping 這個 URL
...
TRADE_CATEGORY_ID = 123456789012345678  # 交易頻道父類別ID
WAIT_TIME = 60
...
# ---------- /買賣交易 指令 ----------
@bot.tree.command(name="買賣交易", description="發布買賣交易訊息")
async def trade(interaction: discord.Interaction):
    ...
    try:
        ...
        await interaction.followup.send("✅ 交易訊息已發布！", ephemeral=True)
    except Exception as e:
        await interaction.followup.send("⚠️ 超時或錯誤，請重新操作。", ephemeral=True)
        print(e)

# ---------- /我要交易 指令 ----------
@bot.tree.command(name="我要交易", description="建立一個新的交易頻道")
