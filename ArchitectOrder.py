...
class ArchitectOrderView(View):
    def __init__(self, client_member: discord.Member, architect_member: discord.Member):
    ...
    async def end_order(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.send_message(
            f"✅ 已結束 {self.client_member.display_name} 的委託，不再顯示此頻道。",
            ephemeral=True
        )
...
class ArchitectBrowseView(View):
    def __init__(self, user: discord.Member, architects: list, architect_data: dict, index: int = 0):
        super().__init__(timeout=None)
        self.user = user
    ...
    async def hire(self, interaction: discord.Interaction, button: Button):
        ...
        await interaction.response.send_message(
            f"✅ 已加入 {arch.display_name} 的建築師頻道：{channel.mention}",
            ephemeral=True
        )
